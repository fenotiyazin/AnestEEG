...
function loadModule(id) {
    ...
    // Render module shell — içerik sonraki aşamada doldurulacak
    contentEl.innerHTML = buildModuleShell(mod);

    // Scroll to top
    document.getElementById('content-area').scrollTop = 0;
    window.scrollTo(0, 0);
    // Simülatör callback — modül 1 sayfa 1 için
    if (id === 1 && typeof onModule1PageLoad === 'function') {
        onModule1PageLoad(1);
    }
}
...
function buildModuleShell(mod) {
    ...
    const nextLabel    = mod.id === 10 ? 'Go to Post-Test →' : `Module ${mod.id + 1}: ${mod.id < 10 ? moduleRegistry[mod.id].title : ''} →`;
    const nextAction   = mod.id === 10 ? 'goToPosttest()' : `completeAndNext(${mod.id})`;
    ...
    <div class="module-bottom-nav">
        <button class="btn-module-nav" onclick="prevModule(${mod.id})" ${prevDisabled}>
            ← Module ${mod.id - 1}
        </button>
        <button class="btn-module-nav primary" onclick="${nextAction}">
            ${nextLabel}
        </button>
    </div>
    `;
...
function switchPage(pageNum, totalPages) {
    ...
    window.scrollTo(0, 0);
    // Simülatör callback
    if (activeModule === 1 && typeof onModule1PageLoad === 'function') {
        onModule1PageLoad(pageNum);
    }
}

// ─── GET PAGE CONTENT ─────────────────────────────────────────────────────────
// Şimdilik placeholder — her modülün içeriği ayrı ayrı yazılacak
function getPageContent(moduleId, pageNum) {
    const contentMap = { 1: module1Content };
